...
public abstract class Animal extends Mobile implements IEdible, IAnimalBehavior,Runnable
, IDrawable {
    private String name;
    ...
    public void foodOnPanel(boolean bool)
    {
        foodOnPanel=bool;
    }
    ...
    public synchronized boolean setWeight(double newWeight)
    {
    ...
    public void loadImages(String nm)
    {
         try
         {
             img1 = ImageIO.read(new File(IDrawable.PICTURE_PATH+nm+"1"+".png"));
             img2=ImageIO.read(new File(IDrawable.PICTURE_PATH+nm+"2"+".png"));
             setChanges(true);
         }
    ...
    public void run()
    {
        ...
        while (running.get())
        {
            ...
            if (!foodOnPanel) {
                if (getLocation().getX() >= 800)
                    x_dir = -1;
                if (getLocation().getX() <= 0)
                    x_dir = 1;
                if (getLocation().getY() >= 600)
                    y_dir = -1;
                if (getLocation().getY() <= 0)
                    y_dir = 1;
                Nx = getLocation().getX() + horSpeed * x_dir;
                Ny = getLocation().getY() + verSpeed * y_dir;
                setLocation(new Point(Nx, Ny));
                setChanges(true);
            }
            else
            {
                if (getLocation().getX() >= 400+getEAT_DISTANCE())
                    x_dir = -1;
                else if (getLocation().getX() <= 400-getEAT_DISTANCE()&&getLocation().getX()>=0)
                    x_dir = 1;
                if (getLocation().getY() >= 300+getEAT_DISTANCE())
                    y_dir = -1;
                else if (getLocation().getY() <=300-getEAT_DISTANCE()&&getLocation().getY()>=0)
                    y_dir = 1;
                Nx = getLocation().getX() + horSpeed * x_dir;
                Ny = getLocation().getY() + verSpeed * y_dir;
                setLocation(new Point(Nx, Ny));
